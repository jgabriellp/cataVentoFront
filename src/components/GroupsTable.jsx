...
const GroupsTable = ({ searchResults = [] }) => {
  ...
  // Buscar grupos por página
  const fetchGroups = async (pageNumber = 1) => {
    if (searchResults.length > 0) return; // não buscar se há resultados da busca
    setLoading(true);
    try {
  ...
  // Atualizar tabela quando houver resultados da busca
  useEffect(() => {
    if (searchResults.length > 0) {
      setGroups(searchResults);
      setHasMore(false); // sem paginação

      // Buscar participantes dos grupos buscados
      searchResults.forEach((g) => fetchParticipants(g.groupId));
    } else {
      fetchGroups(1); // buscar página inicial
    }
  }, [searchResults]);
  ...
  return (
    ...
    >
      ...
      <Table striped bordered hover responsive>
        ...
        <tbody>
          {loading ? (
            ...
            <tr>
              <td colSpan="4" className="text-center" style={{ color: "#888" }}>
      ...
      <div className="d-flex justify-content-between mt-3">
        <Button
          variant="light"
          disabled={page === 1 || loading || searchResults.length > 0}
        ...
        <Button
          variant="light"
          disabled={!hasMore || loading || searchResults.length > 0}
          onClick={() => fetchGroups(page + 1)}
          style={{
            borderRadius: "25px",
            padding: "6px 20px",
